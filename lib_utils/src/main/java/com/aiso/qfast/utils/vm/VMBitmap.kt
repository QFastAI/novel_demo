...
object VMBitmap {
     ...
     */
    @SuppressLint("NewApi")
    fun loadCacheBitmapFromView(view: View, drawingCacheEnabled: Boolean = true): Bitmap? {
        // 启用绘图缓存
    ...
    @SuppressLint("NewApi")
    fun compressAndConvertToBase64(localMedia: LocalMedia, targetQuality: Int = 50): String? {
        return try {
            // 1. 获取文件路径
            val filePath = localMedia.compressPath ?: localMedia.path
            ...
            bitmap.compress(CompressFormat.JPEG, targetQuality, outputStream)

            // 4. 转换为Base64
            val byteArray = outputStream.toByteArray()
            "data:image/png;base64,${Base64.encodeToString(byteArray, Base64.NO_WRAP).also {
